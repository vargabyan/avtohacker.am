...
const Services = ({ home }) => {
  ...
  const dataServiceItem = t('service', { returnObjects: true });
    ...
    <Grid container spacing={4} justifyContent='center' mb={6} mt={1}>
      <Grid item xs={9}>
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <Typography variant='h4' fontWeight={900} align={home ? 'start' : 'center'}>
          ...
          <Grid item xs={12} display={home ? 'block' : 'none'}>
            <Typography fontWeight={900}>ՍՏԱՑԵ՛Ք ԲՈԼՈՐ ԾԱՌԱՅՈՒԹՅՈՒՆՆԵՐԸ՝ 1 ՓԱԹԵԹՈՒՄ</Typography>
          </Grid>
        </Grid>
      </Grid>
      <Grid item xs={9}>
