  ...
  const handelChoicePlace = (e) => {
    ...
    if (e.target.innerText) {
      for (const key in auctionPlace) {
        if (auctionPlace[key].name === e.target.innerText) {
          setAuctionPlacesAndDeliveryPricesState(auctionPlace[key]);
          ...
          <Grid container spacing={2}>
            <Grid item xs={12}>
              <Typography variant="p" color="primary" className="headerText">
                Ստորև ներկայացված հաշվիչը հնարավորություն է տալիս հաշվարկել
                աճուրդի վճարը, ﬔքենայի տեղափոխման արժեքը, ապահովագրությունը և
                մաքսազերծումը՝ 2020 թվականից գործող ԵԱՏՄ սակագներին
                համապատասխան։
              </Typography>
            </Grid>
